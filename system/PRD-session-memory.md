...
5. 팬아웃 — "틴더 수락" 사실이 다섯 집에 거주, 수동 동기화
...
8. **동일 세션 내 망각.** 틴더·리로케이션 논의는 전사 수십 개 분량의 맥락이 **같은 세션 안에**
...
서류철 지정 3개(SOI·리로케이션·레이더) + 복귀/마감 의식 가동 → 정원사 v0
...
정원사 리포트 → 세션이 요약 제시 → 기훈 판정 → 집행 → journal `decision` + DR.
