...
**상태: ACTIVE — 2026-08-20 소유자 판정(D1~D15 전부 채택, /goal) · Phase 1 구현됨.**
이력: v1 2026-08-20 오후(상태 위생) → v2 저녁(깊이 손실·인지 구조·위임·문헌) →
**v3 같은 날, 소유자 요청으로 최종화** — 결정 감사(§8)·스키마 정본(§9)·훅/스킬/핸드오프
...
1. 낡은 상태 단언 — 8/8 메모리가 8/15까지 정본 행세 (수락·사퇴를 소유자가 알려줘야 했음)
...
| 주장의 종류 | 정본 | 왜 |
|---|---|---|
| **소유자가 무엇을 말했나** | 전사 원장 원문 | 그의 발화가 원점이다. 요약·기억은 파생물 |
| **소유자가 무엇을 결정했나** | `system/decisions.md` (DR) + journal `decision` | 발화는 탐색을 포함한다. 결정은 따로 확정된 것만 |
...
그리고 **`role=user`는 소유자의 발화가 아니다.** 세 오염원이 그 라벨을 공유한다.
...
- 소유자 발화의 원문이 필요한 판정(그가 정확히 뭐라 했나)에는 요약이 아니라 **반드시 recall**
...
**산출:** 제안 리포트 1건 (`state/` 아래 `gardener-YYYY-MM-DD.md`). 집행은 세션+소유자 판정 후.
...
### 3.7 임베딩·그래프 — 보류하되 문을 열어둔다 [소유자 제안 검토]
...
유지보수 비용을 상시 부과한다 [한계효용]. ④2026-09 독립 실측: 검색 예산을 고정하면 그래프
메모리가 플랫 검색에 진다 (`2608.28978` Table 1: token F1 0.42 vs 0.47; assistant 원문 회상 judge
0.607 vs 0.911 — 문장을 엔티티로 쪼개며 원문 표현을 버린 탓). 적용 범위: 단일 사용자·대화 회상
과제(LongMemEval) 한정, 다른 과제로 일반화 금지 (DR-052).
...
3. 상태 사실의 손 쓰기 집 5→2
4. **깊이 복원 — 주제 복귀 시 이전 확정 결론·기각 경로를 서류철+recall로 재진술. 소유자가
...
7. **규칙 준수율 — 다이어트 후 상시 규칙 위반이 체감 감소하는가 (소유자 관측 기준)**
...
소유자 선정) + rituals.md (`system/` 아래, P3 예정) 분리 + 규칙별 Why 부착 → START_HERE 온도판 처리(D3) →
...
- **D9. CLAUDE.md 다이어트** — Phase 3으로 분리해뒀다. 상시 코어 7개의 목록 자체를 소유자가
...
- **D13. 스킬 4종** — `/now` `/recall` `/dossier` `/garden` (소유자용 진입점). *제안: Phase 2.*
...
무한 master는 소유자의 인간용 inbox로 유지할 수 있지만, 광역 독해·감사·런타임 토론은
...
- **③계약 변경** (권위 서열, 상시 규칙 세트, 불가침 목록, 인스턴스 소유 문서의 의미 변경,
...
3. 소유자 검수 포인트 명시 — 각 Phase 보고에 "네가 실제로 확인할 것 ≤3개"를 지정
